...
def load_case_from_list_of_files(data_files, seg_file=None,domain=2):
    ...
    if seg_file is not None:   # 加入尾部是否为.nii.gz判断时出错
...
class ImageCropper(object):
    ...
    @staticmethod
    def crop_from_list_of_files(data_files, seg_file=None,domain=2):
        data, seg, properties = load_case_from_list_of_files(data_files, seg_file,domain)
    ...
    def load_crop_save(self, case, case_identifier, overwrite_existing=False):
        try:
            ...
            if overwrite_existing \
                ...
                data, seg, properties = self.crop_from_list_of_files(case[:1], case[-1],len(case))

                # if len(case)==1: # only image
                #     all_data = np.vstack((data))
                # else:
                #     all_data = np.vstack((data, seg))
                all_data = np.vstack((data, seg)) # for image only : seg is generated by foreground
    ...
    def run_cropping(self, list_of_files, overwrite_existing=False, output_folder=None):
        ...
        for j, case in enumerate(list_of_files):
            if case[-1].endswith('.nii.gz'):
                # not only image without label from target domain
                print(case, case[-1])
