...
class ExperimentPlanner(object):
    def __init__(self, folder_with_cropped_data, preprocessed_output_folder, plan_path):
    ...
    def plan_experiment(self):
        ...
        from nnunet.experiment_planning.nnUNet_plan_and_preprocess import load_pregenerated_planfile, plan_file_p
        print('Load predefined plan file or not ? ',load_pregenerated_planfile)
        if load_pregenerated_planfile:
            file = self.plan_path
            print('Load predefined plan file:', file,flush=True)
            plans = pickle.load(open(file, 'rb'))
            for key in plans:
                print(key)
                print(plans[key])
    ...
    def run_preprocessing(self, num_threads):
        ...
        preprocessor = preprocessor_class(normalization_schemes, use_nonzero_mask_for_normalization,
                                         self.plans['transpose_forward'],
                                          intensityproperties)
        target_spacings = [i["current_spacing"] for i in self.plans['plans_per_stage'].values()]
