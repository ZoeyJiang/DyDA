...
def create_lists_from_splitted_dataset(js_file,ptr,pgt):
    lists = []
    p_tr=ptr
    p_gt=pgt
    json_file=js_file
    ...
    for tr in training_files:
        ...
        for mod in range(num_modalities):
            cur_pat.append(join(p_tr, tr['image'].split("/")[-1][:-7] +
                                "_%04.0d.nii.gz" % mod))
        if tr['label']!='None':
            cur_pat.append(join(p_gt, tr['label'].split("/")[-1]))
...
def crop(js_file, ptr, pgt, override=False, num_threads=default_num_threads, cropped_outpath=""):
    ...
    lists, _ = create_lists_from_splitted_dataset(js_file, ptr, pgt)

    imgcrop = ImageCropper(num_threads, output_folder=cropped_outpath)
    imgcrop.run_cropping(lists, overwrite_existing=override)
    shutil.copy(js_file, cropped_outpath)
