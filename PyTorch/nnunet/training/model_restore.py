...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None):
    ...
    trainer = tr(
        nnunet.run.run_training.target_dataset_path_forvali,
        nnunet.run.run_training.train_epochs,
        nnunet.run.run_training.source_training_augmentation,
        nnunet.run.run_training.batchsize_source,
        nnunet.run.run_training.batchsize_target,
        nnunet.run.run_training.gt_p,
        *init)
...
def load_model_and_checkpoint_files(folder, folds=None, mixed_precision=None, checkpoint_name="model_latest"):
    ...
    trainer = restore_model(join(folds, "%s.model.pkl" % checkpoint_name), fp16=mixed_precision)
    # trainer = restore_model(join(folds, "%s.model.pkl" % 'model_ep_1201'), fp16=mixed_precision)
    trainer.output_folder = folder
    trainer.output_folder_base = folder
    trainer.update_fold(0)
    trainer.initialize(False)
    all_best_model_files = [join(folds, "%s.model" % checkpoint_name)]
