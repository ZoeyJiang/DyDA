...
def get_tp_fp_fn_tn(net_output, gt, axes=None, mask=None, square=False):
    """
    :param net_output:net_output must be (b, c, x, y(, z)))
    ...
    with torch.no_grad():
        ...
        if all([i == j for i, j in zip(net_output.shape, gt.shape)]):
            ...
            gt = gt.long()   # change gt from float32 to int64
            y_onehot = torch.zeros(shp_x)    # y_onehot became a tensor value is 0
            if net_output.device.type == "cuda":
                y_onehot = y_onehot.cuda(net_output.device.index)
            y_onehot.scatter_(1, gt, 1)

    tp = net_output * y_onehot
    fp = net_output * (1 - y_onehot) # 出问题的地方
...
# 同时使用Dice和CE损失函数，weight_ce, weight_dice表示两个损失的占比
# TODO:详细了解是否需要归一化
class DC_and_CE_loss(nn.Module):
    def __init__(self, soft_dice_kwargs, ce_kwargs, aggregate="sum", square_dice=False, weight_ce=1, weight_dice=1,
        ...
        # TODO: 增加到文档描述中 square_dice可修改，默认为false
        if not square_dice:
