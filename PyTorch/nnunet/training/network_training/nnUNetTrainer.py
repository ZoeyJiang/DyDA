...
# jzy added
# from nnunet.utils.func import is_source
...
class nnUNetTrainer(NetworkTrainer):
    def __init__(self, gt_p, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
    ...
    # 加载数据集地址为preprocessed文件夹下nnUNetData_plans_v2.1_stage0
    def load_dataset(self):
        self.dataset = load_dataset(self.folder_with_preprocessed_data)
        self.valdataset = load_dataset(self.vali_datasetfolder)
    ...
    #                                                                                    self.bs_target,
    #                                                                                    using_entropy_split=using_entropy_split,
    ...
    def get_basic_generators(self, batch_size_source=1, batch_size_target=1, using_entropy_split=False, txt_path='',
        ...
        self.load_dataset()   # 加载数据
        self.do_split()     # nnUNet 中自带的划分数据集，DyDE中不用

        if self.threeD:  # 使用
            # jzy: load target vali data
            val_keys = list(self.valdataset.keys())
            val_keys.sort()
            self.dataset_val = OrderedDict()
            for i in val_keys:
                self.dataset_val[i] = self.valdataset[i]  # dataset from target domain
            dl_target_val = DataLoader3D(self.dataset_val, self.patch_size, self.patch_size, self.batch_size, False,
                                         oversample_foreground_percent=self.oversample_foreground_percent,
                                         pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r')

            # jzy: from the mixed dataset load source
            if only_ours:
                tr_keys = list(self.dataset.keys())
                tr_keys.sort()
                self.dataset_tr = OrderedDict()
                self.dataset_target = OrderedDict()
                for i in tr_keys:  # case_0041700
                    # if is_source(i):
                    if 'patient' in i: # source dataset
                    # if (int(i[-7:]) < 100000):
                        self.dataset_tr[i] = self.dataset[i]  # dataset from source domain
                    else:
                        self.dataset_target[i] = self.dataset[i] # dataset from target domain

                dl_source = DataLoader3D(self.dataset_tr, self.basic_generator_patch_size, self.patch_size,
                                         batch_size_source, False,
                                         oversample_foreground_percent=self.oversample_foreground_percent,
                                         pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r')
                return dl_source, None, dl_target_val
            else:
                if not using_entropy_split:
                    tr_keys = list(self.dataset.keys())
                    tr_keys.sort()
                    self.dataset_tr = OrderedDict()
                    self.dataset_target = OrderedDict()
                    ...
                    dl_source = DataLoader3D(self.dataset_tr, self.basic_generator_patch_size, self.patch_size,
                                             ...
                                             oversample_foreground_percent=self.oversample_foreground_percent,
                                             pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r')
                    dl_target = DataLoader3D(self.dataset_target, self.basic_generator_patch_size, self.patch_size,
                                             batch_size_target, False,
                                             oversample_foreground_percent=0,
                                             pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r')
                    return dl_source, dl_target, dl_target_val
                else:
                    assert IntraDA3d_lambda is not None
                    print('Using lambda ', IntraDA3d_lambda,flush=True)
                    from nnunet.utilities.entropy_split import jsph_kits_split, acdc_split
                    easy_split, hard_split = jsph_kits_split(txt_path=txt_path, IntraDA3d_lambda=IntraDA3d_lambda)
                    ...
                    for case_id in tr_keys: # case_0041700
                        if With_origin_domain_training:
                            if 'patient' in case_id or case_id in easy_split:
                            # if (int(case_id[-7:]) >= 100000) or case_id in easy_split:
                                self.dataset_tr[case_id] = self.dataset[case_id]  # dataset from source domain
                            elif case_id in hard_split:
                                self.dataset_target[case_id] = self.dataset[case_id]  # dataset from target
                        else:
                            if case_id in easy_split:
                                self.dataset_tr[case_id] = self.dataset[case_id]  # dataset from target domain
                            elif case_id in hard_split:
                                self.dataset_target[case_id] = self.dataset[case_id]  # dataset from source
        ...
        # 2D images
            ...
            dl_target_val = DataLoader2D(self.dataset_val, self.patch_size, self.patch_size, self.batch_size,
                                         oversample_foreground_percent=self.oversample_foreground_percent,
                                         pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r')

            if using_entropy_split:
                with open(txt_path + 'easy_split.txt') as f:
                    self.origin_source = [i_id.strip() for i_id in f]
                with open(txt_path + 'hard_split.txt') as f:
                    self.target_source = [i_id.strip() for i_id in f]
                dl_source = DataLoader2D_IntraDAway(self.dataset, self.origin_source,
                                                    self.basic_generator_patch_size, self.patch_size,
                                                    # self.basic_generator_patch_size,
                                                    # self.batch_size,
                                                    batch_size_source,
                                                    pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r')
                dl_target = DataLoader2D_IntraDAway(self.dataset, self.target_source,
                                                    self.basic_generator_patch_size, self.patch_size,
                                                    # self.basic_generator_patch_size,
                                                    # self.batch_size,
                                                    batch_size_target,
                ...
                # ACN_original Brats2018_dataset
                def split_dataset(data_root, nfold=5, seed=42, select=0):
                    dataset_dir = glob.glob(os.path.join(data_root, "*GG", "Brats18*"))
                    n_patients = len(dataset_dir)
                    ...
                    for i, fold in enumerate(n_fold_list):
                        if i == select:
                            for idx in fold:
                                val_patients_list.append(dataset_dir[idx])
                        else:
                            for idx in fold:
                                train_patients_list.append(dataset_dir[idx])
                ...
                dl_source = DataLoader2D(self.dataset_tr,
                                         self.basic_generator_patch_size, self.patch_size,
                                         # self.basic_generator_patch_size,
                                         # self.batch_size,
                                         batch_size_source,
                                         oversample_foreground_percent=self.oversample_foreground_percent,
                                         pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r')
                dl_target = DataLoader2D(self.dataset_target,
                                         self.basic_generator_patch_size, self.patch_size,
                                         # self.basic_generator_patch_size,
                                         # self.batch_size,
                                         batch_size_target,
    ...
    def finish_online_evaluation(self):
        ...
        # self.all_val_eval_metrics.append((np.mean(global_dc_per_class)*3-0.97)/2)
