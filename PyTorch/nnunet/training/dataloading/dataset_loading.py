...
def unpack_dataset(folder, threads=default_num_threads, key="data",overwrite=False):
    """
    ...
    Args:
        overwrite:
    """
    print('unpack overwrite ',overwrite,flush=True)
    p = Pool(threads)
    npz_files = subfiles(folder, True, None, ".npz", True)
    p.map(convert_to_npy, zip(npz_files, [key] * len(npz_files),[overwrite] * len(npz_files)))
    p.close()
    p.join()
...
class DataLoader3D(SlimDataLoaderBase):
    ...
    def determine_shapes(self):
        ...
        if len(list(self._data.keys())) != 0:  # 处理data为0的情况
...
class DataLoader2D_IntraDAway(SlimDataLoaderBase):
    def __init__(self, data, data_list, patch_size, final_patch_size, batch_size, oversample_foreground_percent=0.0,
                 memmap_mode="r", pseudo_3d_slices=1, pad_mode="edge",
                 pad_kwargs_data=None, pad_sides=None):
        super(DataLoader2D_IntraDAway, self).__init__(data, batch_size, None)
        if pad_kwargs_data is None:
            pad_kwargs_data = OrderedDict()
        self.pad_kwargs_data = pad_kwargs_data
        self.pad_mode = pad_mode
    ...
    def generate_train_batch(self):
        selected_keys = np.random.choice(self.data_list, self.batch_size, True, None)
        # [0014200_232,0009800_7,0014200_231,0029300_1]
        # selected_keys = np.random.choice(self.list_of_keys, self.batch_size, True, None)
        # case_0002100.npz
        source_list = []
        for i in selected_keys:
            source_list.append(('case_' + i.split('_')[0], i.split('_')[1]))

        data = np.zeros(self.data_shape, dtype=np.float32)
        seg = np.zeros(self.seg_shape, dtype=np.float32)

        case_properties = []
        for j, tup in enumerate(source_list):
            (i, z_slide) = tup
            if 'properties' in self._data[i].keys():
                properties = self._data[i]['properties']
            else:
