...
def del_file(filepath):
    ...
    for f in del_list:
        ...
        if os.path.isfile(file_path) and 'dy_entropy_rank_' not in f and 'dy_predict_result_' not in f:
            os.remove(file_path)
...
class NetworkTrainer(object):
    ...
    def run_training(self):
        ...
        print("MAX epoch:", self.max_num_epochs, '--------self epoch:', self.epoch)
        ...
        # After max_num_epochs epoch train
        if self.using_dyncmicDA_after_train:
            print("DyDA epoch sum:", self.dynamicDA_epoch_sum, flush=True)
            self.print_to_log_file('DyDA epoch sum:', self.dynamicDA_epoch_sum,'---', 'DCSS round', self.dynamicDA_period)
            ...
            from nnunet.utilities.json_gen import json_G
            ...
            output_files = [join(self.dynamicDA_temporary_dataset_dir + 'predict_result', i + ".nii.gz") for i in
                            case_ids]
            ...
            # 进行DCSS循环，DCSS轮数 = dynamicDA_epoch_sum // dynamicDA_period
            for x in range(self.dynamicDA_epoch_sum // self.dynamicDA_period):
                ...
                results = []
                torch.cuda.empty_cache()
                preprocessing = preprocess_multithreaded(self, list_of_lists, cleaned_output_files, 6)
                all_output_files = []
                ...
                results = []
                pp_file = join(self.model_dir_path, "postprocessing.json")
                if isfile(pp_file):
                    shutil.copy(pp_file, os.path.abspath(os.path.dirname(output_files[0])))
                    # for_which_classes stores for which of the classes everything but the largest connected component needs to be
                    # removed
                    for_which_classes, min_valid_obj_size = load_postprocessing(pp_file)
                    results.append(pool.starmap_async(load_remove_save,
                                                      zip(output_files, output_files,
                                                          [for_which_classes] * len(output_files),
                                                          [min_valid_obj_size] * len(output_files))))
                    _ = [i.get() for i in results]
                else:
                    print(
                        "WARNING! Cannot run postprocessing because the postprocessing file is missing. Make sure to run "
                        "consolidate_folds in the output folder of the model first!\nThe folder you need to run this in is "
                        "%s" % self.model_dir_path, flush=True)
                pool.close()
                pool.join()
                # process result
                # postprocess    dynamic_predict_name
                postpro_acdc(self.dynamicDA_temporary_dataset_dir + dynamic_predict_name + '/',
                             self.dynamicDA_temporary_dataset_dir + 'postprocess/')

                # entropy rank
                rank_acdc_myo_emidec(True, npz_path=self.dynamicDA_temporary_dataset_dir + dynamic_predict_name,
                                     postprocessd_ct_path=self.dynamicDA_temporary_dataset_dir + 'postprocess',
                ...
                copy_search_file(self.dynamicDA_sourcedomain_dataset+'imagesTr', self.dynamicDA_temporary_dataset_dir + 'dataset/ct')
                copy_search_file(self.dynamicDA_sourcedomain_dataset+'labelsTr', self.dynamicDA_temporary_dataset_dir + 'dataset/gt')
                copy_search_file(self.dynamicDA_targetdomain_ct_dataset, self.dynamicDA_temporary_dataset_dir + 'dataset/ct')
                copy_search_file(self.dynamicDA_temporary_dataset_dir + 'postprocess', self.dynamicDA_temporary_dataset_dir + 'dataset/gt')

                # preprocess
                json_G(self.dynamicDA_temporary_dataset_dir + 'dataset/ct', self.dynamicDA_temporary_dataset_dir + 'dataset/gt', self.dynamicDA_temporary_dataset_dir + 'dataset')
                preprocess_3d(True, self.dynamicDA_temporary_dataset_dir + 'dataset/dataset.json',
                              ...
                              self.dynamicDA_temporary_dataset_dir + 'dataset/preprocess/preprocessed', plan_path=self.init_args[0])

                # Set new dataset dir path, get new generators
                self.reset_dataset_path(self.dynamicDA_temporary_dataset_dir + 'dataset/preprocess/preprocessed/nnUNetData_plans_v2.1_stage0',
                                        ...
                                        self.IntraDA_lambda+(self.dynamicDA_target_lambda-self.IntraDA_lambda)/(self.dynamicDA_epoch_sum // self.dynamicDA_period) * (x+1))

                # Start DCSS training
                for ce in range(self.dynamicDA_period):
                    # 保存DCSS过程中的多轮模型、决策边界、entropy_rank、gt、preproced, cropped,
                    # if ce in [100, 200, 500]:
                    #     copy_search_file()
                    ...
                    with torch.no_grad():
                        ...
                        self.all_val_losses.append(np.mean(val_losses))
                        self.print_to_log_file("target loss: %.4f" % self.all_val_losses[-1])
                    continue_training = self.on_DyDA_epoch_end()
                    epoch_end_time = time()
                    self.epoch += 1
                    self.print_to_log_file("This epoch took %f s\n" % (epoch_end_time - epoch_start_time))
    ...
    # TODO: delete maybe useless manage_patience
    ...
    def on_DyDA_epoch_end(self):
        self.finish_online_evaluation()  # does not have to do anything, but can be used to update self.all_val_eval_
        # metrics

        self.plot_progress()

        # if self.epoch in [1, 5, 10]:
        #     plot_dynamic_boundary(out=self.output_folder, epoch=self.epoch)
